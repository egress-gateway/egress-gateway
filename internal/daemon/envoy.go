...
func prepareEnvoy(c config.Config) (string, error) {
	...
	for _, item := range clusters {
		...
		cluster["type"] = "STATIC"
		cluster["load_assignment"] = map[string]any{"cluster_name": "opa", "endpoints": []any{map[string]any{"lb_endpoints": []any{map[string]any{"endpoint": map[string]any{"address": pipe(filepath.Join(c.RuntimeDir, "opa.sock"))}}}}}}
	}
